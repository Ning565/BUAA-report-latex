# 大学课程报告Latex模版
   ...
   这是一个为北京航空航天大学课程报告编写的 LaTeX 模板，旨在提供一个简洁、美观且易于使用的报告格式，帮助学生快速高效地编写符合各类课程报告。本模版**改编自XDU_Report(普通课程报告封面，含老师)模版，同时参考西安电子科技大学的众多前辈的LaTex报告模版改编而来**。

   本模版在任课老师布置的结课大作业、小作业“无格式要求”时方便使用，无痛编译，轻松上手。
...
```
BUAA-report-latex
	[文件夹]【请先安装字体，否则报错】Fonts      # 存放需要安装的字体，双击安装
	[文件夹] cover                              # 存放校徽校名
	[文件夹] img                                # 存放正文图片
